...
def recommendation(query, tfidf_model, tfidf_matrix, paperTitles):
    ...
    related_docs_indices = np.argsort(cosine_similarities, axis=0)[-10:].reshape((-1, ))[::-1]
...
def get_related_topic(query, nmf_model, tfidf_model, topic_dict, num_top_words=10):
    """Gives the probability of each topic for the user's query
...
def model(search_query, data_dir="./data/"):
    ...
    query = np.array([search_query], dtype=object)

    recommendedList = recommendation(query, vectorizer, vectSum, paperTitles)
    topic_no, _ = get_related_topic(query, nmf_model, vectorizer, topic_dict)
    ...
    plot_count_dict(dict(auth_count.most_common(10)),"Top authors and number of papers","value")
